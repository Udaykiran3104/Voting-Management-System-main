...
Class Action {
	...
	function login(){
		...
		if($qry->num_rows > 0){
		    ...
		    // Check user type
		    if ($type == 1) {
		        // Admin login, no need to check ward status
		        foreach ($user as $key => $value) {
		            if ($key !== 'password') {
			  $_SESSION['login_'.$key] = $value;
		            }
		        }
		        return json_encode(['status' => 'success', 'type' => 1]);
		    } else {
		        // User login, check if the ward is active in the voting application
		        $ward_qry = $this->db->query("SELECT * FROM voting_list WHERE ward = '$ward' AND is_default = 1");
		        if ($ward_qry->num_rows > 0) {
		            // User's ward is active, set session variables
		            foreach ($user as $key => $value) {
			  if ($key !== 'password') {
			      $_SESSION['login_'.$key] = $value;
			  }
		            }
		            return json_encode(['status' => 'success', 'type' => 2]);
		        } else {
		            // User's ward is not active
		            return json_encode(['status' => 'ward_inactive', 'message' => 'Your ward is not active.']);
		        }
		    }
		} else {
		    // Invalid credentials
		    return json_encode(['status' => 'invalid_credentials', 'message' => 'Invalid credentials.']);
		}
	          }
